                                    ...
                                    <td class="px-1 py-5 text-sm w-2/8">
                                        <div class="flex items-center text-sm">
                                            <div class="relative w-10 h-10 mr-3 rounded-full md:block">
                                                ...
                                                @if (isset($order->order_freelancer->detail_user->photo) != null)
                                                    <img class="object-cover w-full h-full rounded"
                                                        src="{{ url(Storage::url($order->order_freelancer->detail_user->photo)) }}"
                                            ...
                                            <div>
                                                <p class="font-medium text-black">
                                                    {{ $order->order_freelancer->name ?? '' }}</p>
                                                <p class="text-sm text-gray-400">
                                                    {{ $order->order_freelancer->detail_user->phone ?? '' }}</p>
                                            </div>
                                        </div>
                                    </td>
                                ...
                                <div class="relative mt-0 md:mt-6">
                                    @if (isset($order->file))
                                        <a href="{{ url(Storage::url($order->file ?? '')) }}"
                                            ...
                                            onclick="return confirm('Are you sure want to download this file?')"> Download
                                            File</a>
                                    @else
                                        <p class="text-sm text-gray-400">
                                            File not available
                            ...
                            <div class="">
                                <div class="p-1 mt-5">
                                    <div class="grid grid-cols-6 gap-6">
                                        <div class="col-span-6">
                                            ...
                                            <textarea placeholder="" type="text" name="service-name" id="service-name"
                                ...
                                <div class="px-1 py-4 text-right">
                                    ...
                                    @if ($order->order_status_id == 2 && isset($order->file))
                                        <a href="{{ route('member.approve.request', $order->id) }}"
                                    ...
                                    @else
                                    <a type="button"
                                        class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-lg shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500" disabled>
                                        Approve
                                    </a>
                                    @endif
                                </div>
